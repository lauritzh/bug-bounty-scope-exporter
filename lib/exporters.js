...
const sectionLabels = [
  ...
  ["rewards", "Rewards fingerprint"],
  ["program", "Program state fingerprint"],
];

const sectionNames = {
  assets: "the scope assets",
  policy: "the policy text and attachments",
  rewards: "the rewards",
  program: "the program state",
};

const updateLabels = {
  latest: "Latest platform change",
  policy: "Policy updated",
  scope: "Scope updated",
  assets: "Assets updated",
  rewards: "Rewards updated",
  declarations: "Declarations updated",
  program: "Program updated",
  inScope: "In-scope guidance updated",
  outOfScope: "Out-of-scope guidance updated",
  faq: "FAQ updated",
  severityAssessment: "Severity assessment updated",
  rulesOfEngagement: "Rules of engagement updated",
};
...
function addCapture(lines, capture) {
  if (!capture || typeof capture !== "object") return;

  const rows = [];
  if (capture.retrievedAt) rows.push(["Retrieved at", capture.retrievedAt]);
  if (capture.retrievedFrom) rows.push(["Retrieved from", plainUrl(capture.retrievedFrom)]);
  if (capture.source?.description || capture.source?.method) {
    const method = [capture.source.description, capture.source.method ? `(${capture.source.method})` : ""]
      .filter(Boolean)
      .join(" ");
    rows.push(["Retrieved via", method]);
  }
  if (capture.source?.endpoint) rows.push(["Retrieval endpoint", plainUrl(capture.source.endpoint)]);
  if (capture.source?.complete === false) rows.push(["Coverage", "Partial — see notes"]);

  const updates = capture.platformUpdatedAt && typeof capture.platformUpdatedAt === "object"
    ? capture.platformUpdatedAt
    : {};
  const { latest, ...sections } = updates;
  if (latest) rows.push([updateLabels.latest, latest]);
  for (const [key, value] of Object.entries(sections)) {
    if (value) rows.push([updateLabels[key] || `${humanizeKey(key)} updated`, value]);
  }

  const fingerprint = capture.fingerprint;
  if (fingerprint?.overall) {
    const algorithm = fingerprint.algorithm || "sha-256";
    rows.push([`Content fingerprint (${algorithm})`, fingerprint.overall]);
    for (const [key, label] of sectionLabels) {
      const sectionDigest = fingerprint.sections?.[key];
      if (sectionDigest === undefined) continue;
      rows.push([label, sectionDigest || "Not captured"]);
    }
  }
  if (fingerprint?.counts) {
    rows.push(["In-scope assets", fingerprint.counts.inScope]);
    rows.push(["Out-of-scope assets", fingerprint.counts.outOfScope]);
  }
  ...
  if (!fingerprint?.overall) return;

  const missing = Array.isArray(fingerprint.coverage?.missing) ? fingerprint.coverage.missing : [];
  lines.push(
    "Scope and policy can change at any time. Re-export and compare the fingerprints above to detect a change." +
    " They cover the scope assets, the policy text and its attachments, the rewards, and the program state," +
    " and ignore counters, identifiers, ordering, whitespace, and platform timestamps.",
    "",
  );

  if (fingerprint.coverage?.complete === false) {
    const uncaptured = missing.map((section) => sectionNames[section] || section);
    lines.push(
      `**This snapshot is incomplete.** ${uncaptured.length > 0
        ? `It does not contain ${listPhrase(uncaptured)}.`
        : "Part of the program could not be retrieved."}` +
      " The fingerprints above therefore cover only what was retrieved, and an unchanged fingerprint does not rule" +
      " out a change to content this export could not read. Re-export from the live program page before relying on it.",
      "",
    );
    return;
  }
  ...
  if (missing.length > 0) {
    lines.push(
      `This program declares no ${listPhrase(missing.map((section) => sectionNames[section] || section))},` +
      " so those fingerprints are empty.",
      "",
    );
    return;
  }
  ...
  lines.push(
    "An unchanged overall fingerprint means the content covered above is unchanged.",
    "",
  );
}
...
export function toMarkdown(scope, { platformLabel = scope.platform, snapshot = true } = {}) {
  const title = scope.program.name || scope.program.handle || "Program Scope";
  const lines = [
    `# ${escapeMarkdown(title)}`,
    "",
    `Platform: ${escapeMarkdown(platformLabel)}`,
    `Program: ${plainUrl(scope.program.url)}`,
  ];
  if (scope.capture?.retrievedAt) {
    const from = scope.capture.retrievedFrom && scope.capture.retrievedFrom !== scope.program.url
      ? ` from ${inlineCode(plainUrl(scope.capture.retrievedFrom))}`
      : "";
    lines.push(`Retrieved: ${escapeMarkdown(scope.capture.retrievedAt)}${from}`);
  }
  lines.push("");

  if (snapshot) addCapture(lines, scope.capture);

  const { updatedAt, lastPolicyChangeAt, lastScopeChangeAt, lastUpdatedAt, ...metadata } = scope.metadata || {};
  // Platform timestamps are only redundant here while the Snapshot section carries them.
  const hasCapturedUpdates = snapshot && Object.keys(scope.capture?.platformUpdatedAt || {}).length > 0;
  addMetadataSection(lines, "Program Metadata", {
    website: scope.program.website,
    // The Snapshot section above is the single source of freshness.
    ...(hasCapturedUpdates ? metadata : scope.metadata),
  });
